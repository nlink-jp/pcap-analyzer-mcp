...
- **`analysis_failed` + `operation not permitted` from `extract_objects` is the host's antivirus**, not us — it quarantines the sample mid-write and one quarantined object fails the whole call. Defang covers accidental execution and filename-borne attacks, **not** detection: AV matches content, and that detection is a true positive, so suppressing it would be evasion. The workaround is an AV exclusion on the workspace, which **removes protection and leaves live malware unquarantined** — see `docs/{en,ja}/reference/field-notes*.md` before recommending it.
