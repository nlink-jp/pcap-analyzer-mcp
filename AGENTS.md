...
- **Whether a capture exists never changes the answer.** `workspace.ResolveInput` places `pcap_path` first (`workdir.Where`, the last of pathguard's forms: every link followed, a dangling one by its target) and judges it there, as given and as placed (`refused`), before `EvalSymlinks(where)` asks whether it exists; if it then resolves elsewhere it is judged again. A missing credential file used to answer `pcap_unreadable` and an existing one `path_not_allowed`. Do not give a path that does not resolve a branch of its own, and do not stat before the floor (ADR-0010, amendment v0.6.1). `TestExistenceIsNotRevealed` (internal/tools) compares the whole `create_workspace` answer for a path with and without its file; `TestPlacementCorners` pins a link climbing past a file and a loop. Four mutations of the order were all caught by assertion.
