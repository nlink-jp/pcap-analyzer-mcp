...
`pcap-analyzer-mcp` は、バージョンを固定した tshark をコンテナに閉じ込め、解析対象の pcap を read-only でマウントして解析し、~~**結果が小さければインライン JSON、大きければワークスペースへ JSONL ファイル**~~ → 2026-09-13 改訂（ADR-0009）: **結果は行数上限とバイト上限のもとでレスポンスに載せて返し、上限が落とした分は計上する**。ワークスペースに残るのは `extract_objects` が取り出したオブジェクトだけ、という統一契約で返す MCP サーバーである。これによりエージェントは、セキュリティインシデント調査やネットワーク障害解析において、GB 級のパケットキャプチャを段階的に絞り込みながら分析できるようになる。
...
| ADR-0004 | 1 pcap : 1 workspace + ro マウント（コピーしない、`workspace_dir` は呼び出し時指定 — ADR-0008 で `work_dir` に改名） |
| ADR-0005 | 出力契約（バイト閾値 / JSONL / `matched`・`sample` 常時 / 形状不変） — ファイル媒介の半分は ADR-0009 で撤回 |
