...
- 設定キー `workspace.allowed_paths` は消す。**キーが残った config は大声で落とす**
  ...
  書いたつもりのガードが無言で消える
- **判定は「渡された綴り」と「symlink 解決後」の両方で行う。** 実測で決まった:
  このマシンの `~/.ssh/config` は Dropbox 実体へのシンボリックリンクで、解決してから
  照合するとブラックリストを素通りする（もう `~/.ssh` 配下に見えない）。逆に未解決
  だけで照合すると、普通のディレクトリに張ったリンクで通り抜けられる。パス側の
  両方の綴りを、ブラックリスト項目側の両方の綴り（項目自体が symlink の場合がある）
  と突き合わせる
- ブラックリストは**床であって境界ではない**。同じ中身がリストの外にも存在するとき
  （上の例なら Dropbox 側の実体を直接名指ししたとき）は止められない。プロセスが
  何に触れうるかを縛るのはサンドボックス MCP プロキシの仕事で、そちらは意図的に
  後回しにしている
