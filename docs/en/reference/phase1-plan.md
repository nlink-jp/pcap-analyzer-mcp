...
- A synthetic capture generator (`samples/generate.sh`) with four committed outputs
  - `web-session.pcapng` — a plain HTTP exchange
  - `suspicious-download.pcapng` — an HTTP response whose body attempts an injection
  - `mixed.pcapng` — the two merged, giving two conversations
  - `truncated.pcapng` — cut with `editcap -s 40`
- Dummy MCP client E2E harness (`e2e/`, `-tags e2e`) with eleven scenario stages
- `samples/README.md` — the same eleven stages as a human-facing graded walkthrough

**gopacket was not used.** The plan assumed it, but the analysis image already
ships the Wireshark suite, so `text2pcap` / `mergecap` / `editcap` do the job —
no new Go dependency, and fixtures are **generated by the same tshark build
