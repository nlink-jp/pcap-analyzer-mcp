...
| `extract_objects` returns `analysis_failed` with `operation not permitted` | The host's AV grabbed and quarantined the malware mid-write. Not a tool bug — the extraction itself succeeded. One quarantined object fails the whole call. See [Field notes](field-notes.md); an AV exclusion works around it but **removes protection and carries its own risk** |
