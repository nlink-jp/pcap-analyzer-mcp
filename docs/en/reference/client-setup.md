...
The graded walkthrough in [`samples/README.md`][samples]
...
| `extract_objects` reports objects under `skipped` with `operation not permitted` | The host's AV grabbed and quarantined the malware mid-write. Not a tool bug — the other objects still come back, and the skip itself is a finding. See [Field notes](field-notes.md); an AV exclusion recovers the sample but **removes protection and carries its own risk** |
| No `.exe` files come out of an `ftp-data` export | tshark's ftp-data dissector does not export binary `RETR` transfers. Unrelated to AV. A `ftp.request.command` query still yields filenames and sizes. See [Field notes](field-notes.md) |
...
- [Tips](tips.md) — the shape of an investigation, per-protocol filter recipes, reading the results
- [Field notes](field-notes.md) — AV behaviour with real malware, the risk of exclusions, and the ftp-data limitation
